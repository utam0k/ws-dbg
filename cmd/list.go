...
var listCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		header := []string{"ID", "Uptime", "Cpu(%)", "CpuLimit", "Memory(%)", "MemoryStat", "CgroupPath"}
		...
		for _, ws := range wss {
			...
			table.Append([]string{ws.Id, ws.Uptime.String(), fmt.Sprintf("%.2f", cpuUsage), ws.CpuMax.String(), fmt.Sprintf("%.2f", memUsage), ws.MemoryStat.String(), ws.CgroupPath})
		}
		table.Render()
	},
}
